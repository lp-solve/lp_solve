...
MYBOOL __WINAPI set_upbo(lprec *lp, int colnr, REAL value)
{
  ...
  if(lp->tighten_on_set) {
    if(value < lp->orig_lowbo[colnr]) {
      report(lp, IMPORTANT, "set_upbo: Upperbound must be >= lowerbound\n");
      return(FALSE);
    }
    if(value < lp->orig_upbo[colnr]) {
      set_action(&lp->spx_action, ACTION_REBASE);
      lp->orig_upbo[colnr] = value;
    }
  }
  else
  {
    ...
    if(value >= lp->infinite)
    {
      ...
      if (lp->basis_valid && !lp->is_basic[colnr] && !lp->is_lower[colnr]) {
          /* lp->basis_valid = FALSE; */
          lp->is_lower[colnr] = TRUE;
      }
    }
    if (value < lp->infinite && lp->orig_lowbo[colnr] > -lp->infinite && value != lp->orig_lowbo[colnr] && fabs(value - lp->orig_lowbo[colnr]) < lp->epsvalue)
      value = lp->orig_lowbo[colnr];
    lp->orig_upbo[colnr] = value;
  }
  return(TRUE);
}
...
MYBOOL __WINAPI set_lowbo(lprec *lp, int colnr, REAL value)
{
  ...
  if(lp->tighten_on_set) {
    ...
    if((value < 0) || (value > lp->orig_lowbo[colnr])) {
      set_action(&lp->spx_action, ACTION_REBASE);
      lp->orig_lowbo[colnr] = value;
    }
  }
  else
  {
    ...
    if (value > -lp->infinite && lp->orig_upbo[colnr] < lp->infinite && value != lp->orig_upbo[colnr] && fabs(value - lp->orig_upbo[colnr]) < lp->epsvalue)
      value = lp->orig_upbo[colnr];
    lp->orig_lowbo[colnr] = value;
  }
  return(TRUE);
}
